...
const USER_AGENT = `Amersfoort bot ${require('./package.json').version} , Node ${process.version} (${process.platform}${process.arch})`;
...
exports.start = function(SETUP) {
  ...
  const sendOrUpdate = function(embed) {
    ...
    let dot = TICK_N % 2 === 0 ? 'Amersfoort' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor(" Amersfoort Status", "https://i.imgur.com/smuhJWw.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? ' 💻 Amersfoort' : ' 👀 Amersfoort')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van "connect 127.0.0.1" in je F8 te typen.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
 ...
 bot.on('ready',() => {
    ...
    bot.user.setActivity("\Amersfoort", { type: "PLAYING" });
